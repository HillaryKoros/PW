...
export default function OutbreakStagesLayer({ 
  ...
  useEffect(() => {
    ...
    if (visible && !layerRef.current) {
      // Create WMS layer for outbreak stages data through MapServer
      const wmsLayer = L.tileLayer.wms(getMapServerUrl(), {
        ...
        opacity: opacity,
        crs: L.CRS.EPSG4326,
        attribution: 'Locust Outbreak Stages - MapServer WMS'
      });

      // Add click event for outbreak stages info
      wmsLayer.on('click', (e) => {
        const popup = L.popup()
          .setLatLng(e.latlng)
          .setContent(`
            <div class="p-2">
              <h3 class="font-semibold text-sm">Locust Outbreak Stages</h3>
              <p class="text-xs text-gray-600">Location: ${e.latlng.lat.toFixed(4)}, ${e.latlng.lng.toFixed(4)}</p>
              <p class="text-xs text-gray-600">Phase classification and density information</p>
            </div>
          `)
          .openOn(map);
      });
